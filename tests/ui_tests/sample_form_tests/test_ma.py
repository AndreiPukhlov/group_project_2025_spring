...
from tests.ui_tests.sample_form_tests.test_ty import REQUIRED_ERROR_MESSAGES
...
#empty field error locators
NAME_FIELD_ERROR = (By.ID, "name-error")
USER_NAME_FIELD_ERROR = (By.ID, "username-error")
PASSWORD_FIELD_ERROR = (By.ID, "password-error")
EMAIL_FIELD_ERROR = (By.ID, "email-error")

REQUIRED_ERROR_MESSAGE = [NAME_FIELD_ERROR, USER_NAME_FIELD_ERROR, PASSWORD_FIELD_ERROR, EMAIL_FIELD_ERROR]

THIRD_PARTY_AGREEMENT_BUTTON_ERROR = (By.ID, "agreedToPrivacyPolicy-error")
...
class TestSampleForm:
    ...
    def test_empty_all_required_fields_by_id(self, driver):
        page_sp = SampleFormPage(driver, url)
        page_sp.open()
        page_sp.element_is_visible(FORM_SUBMIT_BUTTON).click()
        logger.info("Clicked submit button without filling out required fields")

        # name_error_message = page_sp.element_is_visible(NAME_FIELD_ERROR).text
        # logger.info("Found name field error")
        # username_error_message = page_sp.element_is_visible(USER_NAME_FIELD_ERROR).text
        # logger.info("Found user name field error")
        # password_error_message = page_sp.element_is_visible(PASSWORD_FIELD_ERROR).text
        # logger.info("Found password field error")
        # email_error_message = page_sp.element_is_visible(EMAIL_FIELD_ERROR).text
        # logger.info("Found email field error")
        thirdpartyagreement_error_message = page_sp.element_is_visible(THIRD_PARTY_AGREEMENT_BUTTON_ERROR).text
        # logger.info("Found third party agreement button error")
        #
        # assert name_error_message == "Found name field error"
        # assert username_error_message == "Found name field error"
        # assert password_error_message == "Found name field error"
        # assert email_error_message == "Found name field error"
        confirm_password_field = page_sp.element_is_visible(CONFIRM_PASSWORD_FIELD)

        assert all(page_sp.element_is_visible(error_locator).text == "This field is required."
